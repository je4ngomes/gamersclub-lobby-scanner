...
const getAllPlayerInfo = (card) => {
  ...
  fetchResource(playerURL)
    .then((response) => {
      return getSteamIDByResponse(response);
    })
    .then((steamId) => {
      getStorageData("api_key")
        .then((response) => {
        ...
        .then((endpoints) => {
          Promise.all(endpoints.map((endpoint) => fetchResource(endpoint)))
            .then((responses) =>
              responses.map((response) => parseToJSON(response))
            )
            .then(
              ([
                GetFriendList,
...
const handleGetPlayerSummaries = (json) => {
  let array = [];
  let visibility = [1, 2].includes(
    json.response.players[0].communityvisibilitystate
  )
    ? blockedSpan("Sim")
    : "Não";
...
const handleGetOwnedGames = (json) => {
  ...
  if (playedTime.playtime_forever == 0) {
    ...
    array.push(
      data("Horas jogadas", (playedTime.playtime_forever / 60).toFixed(2))
    );
  }
  ...
  mutations.forEach((mutation) => {
    ...
    mutation.addedNodes.forEach((node) => {
      if (
        ...
        !node.classList.contains("sala-card-wrapper")
      ) {
        return false;
      }
      ...
      players.forEach((card) => {
        card.addEventListener("mouseover", (e) => {
          ...
          if (
            ["loaded", "loading"].includes(
              card.getAttribute("data-info-loaded")
            )
          ) {
            return;
          }
